...
# 改用我們新定義的兩個函式
from analyzer import calculate_sctr_ranks, check_vcp_strict, check_vcp_loose
...
if st.sidebar.button("🚀 執行全球同步掃描"):
    ...
    if res_tuple[0]:
        ...
        st.write(f"正在掃描 {market_name} ... 使用 **{screen_mode}**")
        ...
        for i, t in enumerate(tickers):
            if screen_mode.startswith("嚴苛"):
                res = check_vcp_strict(t, sctr_ranks, sctr_hist, only_b, b_days)
            else:
                res = check_vcp_loose(t, sctr_ranks, sctr_hist, only_b, b_days)
            
            if res and res[3] >= min_sctr_val:   # res[3] 是 SCTR排名
                results.append(res)
        ...
        if results:
            df = pd.DataFrame(results, columns=[
                "代碼", "價格", "距離高點%", "SCTR排名", "收縮狀態", "量比", 
                "狀態", "行業", "Pivot(樞軸)", "SL(ATR停損)", "Target(目標3R)"
            ])
            ...
            st.success(f"掃描完成！共找到 {len(df)} 檔符合條件的標的（{screen_mode}）")
        else:
            st.warning("今日未篩選出符合條件的標的。")
    else:
