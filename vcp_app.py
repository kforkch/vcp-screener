...
def check_vcp_advanced(ticker, sctr_map, b_only, b_days):
    try:
        ...
        if sum(cond) == 6:
            # 只有符合 VCP 形態後才抓取行業，節省 API 請求時間
            t_info = yf.Ticker(ticker).info
            sector = t_info.get('sector', '其他/未知')
...
if st.sidebar.button("🚀 執行全球同步掃描"):
    ...
    if res_tuple[0]:
        ...
        st.write("---")
        st.info(f"📊 正在掃描 {market_name}... (預計 60-90 秒)")
        ...
        if results:
            ...
            # 1. 產業熱點統計
            st.subheader("🔥 產業熱點分佈 (Sector Analysis)")
            ...
            with col_stat:
                ...
                st.caption("提示：當特定行業入選標的增加，代表資金正流向該板塊。")
            ...
            st.dataframe(df.sort_values("SCTR排名", ascending=False), 
