...
def get_stock_list(market):
    # 這裡直接回傳設定好的清單，不需任何外部請求
    index_map = {
        "美股 (S&P 500)": "^GSPC",
        "港股 (恒生指數)": "^HSI",
        "中國 A 股 (滬深 300 龍頭)": "000300.SS"
    }
    return STOCKS_UNIVERSE.get(market, []), index_map.get(market)

@st.cache_data(ttl=86400)
def get_sector_cached(ticker):
    try:
        ticker_obj = yf.Ticker(ticker)
        return ticker_obj.info.get('sector', 'N/A')
    except:
        return 'N/A'
